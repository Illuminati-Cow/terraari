...
public class HydrolysistBossBody : ModNPC
{
    ...
    public override void AI()
    {
		...
		float num10 = 5600f;
		if (NPC.target < 0 || NPC.target == 255 || player.dead || !player.active || Vector2.Distance(player.Center, center) > num10)
		{
			...
			if (NPC.ai[1] >= 4f && flag2 && (int)(NPC.ai[1] - 4f) % num == 0)
			{
					...
					foreach (int item3 in list3)
					{
						...
						if (Main.netMode != NetmodeID.MultiplayerClient)
						{
							...
							for (int m = 0; m < 1; m++)
							{
								...
								Projectile.NewProjectile(NPC.GetSource_FromAI(), vector3.X, vector3.Y, spinninpoint.X, spinninpoint.Y, 468, 18, 0f, Main.myPlayer);
							}
						}
					}
				}
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					for (int n = 0; n < 1; n++)
					{
						...
						Projectile.NewProjectile(NPC.GetSource_FromAI(), vector4.X, vector4.Y, vector5.X, vector5.Y, 464, attackDamage_ForProjectiles, 0f, Main.myPlayer, 0f, 1f);
					}
				}
			}
			NPC.ai[1] += 1f;
			...
			if (NPC.ai[1] >= 4f && flag2 && (int)(NPC.ai[1] - 4f) % num2 == 0)
			{
						...
						foreach (int item4 in list4)
						{
							...
							if (Main.netMode != NetmodeID.MultiplayerClient)
							{
								...
								for (int num20 = 0; num20 < 1; num20++)
								{
									...
									Projectile.NewProjectile(NPC.GetSource_FromAI(), vector6.X, vector6.Y, spinninpoint2.X, spinninpoint2.Y, ProjectileID.CultistBossFireBallClone, 18, 0f, Main.myPlayer);
								}
							}
						}
					}
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					for (int num22 = 0; num22 < 1; num22++)
					{
						...
						Projectile.NewProjectile(NPC.GetSource_FromAI(), vector7.X, vector7.Y, spinninpoint3.X, spinninpoint3.Y, ProjectileID.CultistBossFireBall, attackDamage_ForProjectiles2, 0f, Main.myPlayer);
					}
				}
			}
			NPC.ai[1] += 1f;
				...
				foreach (int item5 in list5)
				{
					...
					if (Main.netMode != 1)
					{
						...
						for (int num25 = 0; num25 < 1; num25++)
						{
							...
							Projectile.NewProjectile(NPC.GetSource_FromAI(), vector9.X, vector9.Y, spinninpoint4.X, spinninpoint4.Y, 468, 18, 0f, Main.myPlayer);
						}
					}
				}
				if ((int)(NPC.ai[1] - 20f) % num4 == 0)
				{
					Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y - 100f, 0f, 0f, 465, attackDamage_ForProjectiles3, 0f, Main.myPlayer);
				}
			}
			NPC.ai[1] += 1f;
				...
				if (NPC.ai[1] == 30f && Main.netMode != 1 && flag2)
				{
					...
					for (int num34 = 0; num34 < array.Length; num34++)
					{
						if (num31 != num34)
						{
							...
							if (num33-- > 0)
							{
								int num35 = NPC.NewNPC(NPC.GetSource_FromAI(), (int)center6.X, (int)center6.Y + NPC.height / 2, 440, NPC.whoAmI);
		...
		else if (NPC.ai[0] == 7f)
		{
			...
			if (NPC.ai[1] >= 4f && flag2 && (int)(NPC.ai[1] - 4f) % num5 == 0)
			{
				if ((int)(NPC.ai[1] - 4f) / num5 == 2)
				{
					...
					{
						if (Main.npc[num42].active && Main.npc[num42].type == 440 && Main.npc[num42].ai[3] == (float)NPC.whoAmI)
						{
							list7.Add(num42);
						}
					}
					foreach (int item6 in list7)
					{
						...
						if (Main.netMode != 1)
						{
							...
							for (int num44 = 0; (float)num44 < 5f; num44++)
							{
								...
								Projectile.NewProjectile(NPC.GetSource_FromAI(), vector14.X, vector14.Y, spinninpoint5.X, spinninpoint5.Y, 468, 18, 0f, Main.myPlayer);
							}
						}
					}
				}
				...
				if (Main.netMode != 1)
				{
					...
					for (int num48 = 0; (float)num48 < 5f; num48++)
					{
						...
						int num49 = NPC.NewNPC(NPC.GetSource_FromAI(), (int)vector15.X, (int)vector15.Y + 7, 522, 0, 0f, ai, spinningpoint2.X, spinningpoint2.Y);
		...
		else if (NPC.ai[0] == 8f)
		{
			...
			if (NPC.ai[1] >= 4f && flag2 && (int)(NPC.ai[1] - 4f) % num7 == 0)
			{
				...
				{
					if (Main.npc[num50].active && Main.npc[num50].type == 440 && Main.npc[num50].ai[3] == (float)NPC.whoAmI)
					{
						list8.Add(num50);
					}
				}
